...
import { mockEmails, mockStudents, Email } from "@/data/mockData";
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Select value={composeTo} onValueChange={setComposeTo}>
              ...
              <SelectContent>
                {mockStudents.map((s) => (
                  <SelectItem key={s.id} value={s.id}>{s.fullName} ({s.admissionNumber})</SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <div className="flex justify-between">
              <Button variant="outline" size="sm"><Paperclip className="h-3.5 w-3.5 mr-1" /> Attach</Button>
          ...
          <TabsContent key={tab} value={tab}>
            <ScrollArea className="h-[400px]">
              {getEmailsForTab().length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-12">No emails.</p>
